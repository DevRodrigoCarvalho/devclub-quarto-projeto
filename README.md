<div align="center" style="vertical-align: middle; margin: 0; padding: 0;">
  <img height="250" alt="image" src="https://github.com/DevRodrigoCarvalho/img-para-utilizar-no-github/blob/main/logo-red-dragon-sem-fundo.png"/>
  <h1>
    Number Randomizer | Sorteador Numérico
  </h1>
</div>

<p align="center">
<img height="400vw" src="https://github.com/DevRodrigoCarvalho/devclub-quarto-projeto/blob/0aa75a1300a379542fc6a8f59a0f9eef0ef8336e/assets/readme-image.png" alt="logo_currency_converter"/>
</p>

## <img height="20" alt="image" src="https://github.com/user-attachments/assets/d590403e-d286-4bf8-b2df-baebce0d40c1"/> Sobre o projeto
<p>
  Este projeto consiste em um sorteador numérico interativo desenvolvido com foco em experiência do usuário.
  A proposta vai além de apenas gerar números aleatórios, trazendo uma animação estilo cassino 🎰, onde os valores "rolam" antes de revelar o resultado final.
  <br/>
  O objetivo foi transformar uma aplicação simples em algo mais dinâmico, visualmente atrativo e com melhor feedback para o usuário.
</p>

---

## <img width="30" alt="image" src="https://github.com/user-attachments/assets/1a95fe4e-18d1-441b-852b-366af0c20b74"/> Funcionalidades
<div>
  <ul>
    <li>Geração de número aleatório entre valores definidos</li>
    <li>Validação de entrada (mínimo e máximo)</li>
    <li>Animação de "rolagem" dos números antes do resultado</li>
    <li>Transição suave entre imagem e resultado</li>
    <li>Feedback visual durante a interação</li>
  </ul>
</div>

---

## <img width="28" alt="image" src="https://github.com/user-attachments/assets/44ee562e-a94d-418a-9c32-45a881244b3d"/> Acesse ao projeto
Deploy: Em manutenção!
<br/>
GitHub: Em manutenção!

---

## <img height="30" alt="image" src="https://github.com/user-attachments/assets/d5c75e84-490d-4ee0-ae82-f3b8a126f143"/> Demonstração
<div>
  <p>Interface intuitiva onde o usuário pode:</p>
  <ul>
    <li>Digite o Mínimo</li>
    <li>Digite o Máximo</li>
    <li>Clique em Sortear</li>
    <li>Veja a mágia acontecer!</li>
  </ul>
</div>

---

## <img height="30" alt="image" src="https://github.com/user-attachments/assets/b1bb008e-1d32-4827-a6e3-8fa1159598c6"/> Tecnologias utilizadas
<div>
  <ul>
    <li>HTML5</li>
    <li>CSS3</li>
    <li>JavaScript</li>
  </ul>
</div>

---

## <img height="25" alt="image" src="https://github.com/user-attachments/assets/058ccb82-9777-4d37-a798-735e2f017ee5"/> Responsividade
<div>
  <p>
    O projeto foi desenvolvido com foco em responsividade, garantindo uma boa experiência em:
  </p>
  <ul>
    <li><img width="20" alt="image" src="https://github.com/user-attachments/assets/866d24c1-72b9-4470-be3f-7e434eb9345b"/>Desktop</li>
    <li><img width="20" alt="image" src="https://github.com/user-attachments/assets/5b85d4ff-e219-45d0-b87d-b73f4309d75e"/>Tablet</li>
    <li><img width="20" alt="image" src="https://github.com/user-attachments/assets/5d0492ea-1a4f-433d-b002-98f23c4d5784"/>Mobile</li>
  </ul>
</div>

---

## <img height="28" alt="image" src="https://github.com/user-attachments/assets/ae06d080-48b2-42b5-9411-900b52334ddc"/> Aprendizados
<div>
  <p>Durante o desenvolvimento deste projeto, foram aplicados conceitos importantes como:</p>
  <ul>
    <li>Manipulação do DOM</li>
    <li>Controle de tempo com setInterval e setTimeout</li>
    <li>Criação de animações com CSS (opacity e transform)</li>
    <li>Estruturação e organização de código</li>
    <li>Tratamento de valores numéricos</li>
    <li>Melhoria da experiência do usuário (UX)</li>
  </ul>
</div>

---

## <img height="30" alt="image" src="https://github.com/user-attachments/assets/f1eb8107-2736-4a8f-ab97-93d53ecd7b6d"/> Desafios
<div>
  <ul>
    <li>Criar um efeito de animação fluido para simular o "rolar" dos números</li>
    <li>Garantir que a transição entre imagem e resultado fosse suave</li>
    <li>Evitar conflitos ao clicar várias vezes no botão</li>
    <li>Manter o layout responsivo sem quebrar a interface</li>
  </ul>
</div>

---

## <img width="25" alt="image" src="https://github.com/user-attachments/assets/3ba58ebc-3d03-40d9-85f6-d1408d4b8165"/> Autor
<p>
  Feito por <i>Rodrigo Carvalho</i>
  <br/>
  <img width="15" alt="image" src="https://github.com/user-attachments/assets/191e0cf7-cbf7-40fe-912c-30253c4e301a"/> Em transição para a área de desenvolvimento Front-End
</p>
